happyLeaf.factory('dataManager', ['$rootScope', '$localStorage', 'logManager', function($rootScope, $localStorage, logManager){
     ...
     if(splitMsg.length < 7){
       ...
       if(batteryTempC > 46) {
         $rootScope.$broadcast('notification', {
           ...
           seen: false,
           content: "<h1>High Battery temperature</h1><p>High battery temperatures can cause irriversable damage, avoid quick charging until the battery has cooled.<br/>Your battery temperature was read at: "+self.batteryTemp+"&deg. </p>",
           icon: "whatshot"
         });
       }
       ...
       if(batteryTempC < 0) {
         $rootScope.$broadcast('notification', {
           ...
           time: (new Date()).getTime(),
           seen: false,
           content: "<h1>Low Battery temperature</h1><p>Low battery temperatures can cause irriversable damage. Connect to a charger to activate battery heater. <br/>Your battery temperature was read at: "+self.batteryTemp+"&deg. </p>",
           icon: "ac_unit"
         });
       }
